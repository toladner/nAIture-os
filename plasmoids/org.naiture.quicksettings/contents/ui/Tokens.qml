...
QtObject {
    ...
    // The panel background's margin, from tools/make_panel_svg.py. An applet
    // is laid out inside it, so a marker that belongs to the island's edge has
    // to reach back out by exactly this much. Keep the two in step.
    ...
    // The chevron that leads out of a tile into its settings.
    readonly property int chevronSize: 24
